    ...
    if mode in ("long", "both"):
        ...
        if asset_class == "crypto":
            ic_context = (
                ...
                f"══ AUTHORITY DATA ══  (cite ONLY from this block — do not invent or recall values)\n"
                f"Price (USD):          {price}\n"
                f"Market Cap:           {fmt_number(info.get('market_cap'))}\n"
                f"Market Cap Rank:      #{info.get('market_cap_rank','not in data')}\n"
                f"24H Change:           {info.get('price_change_24h_pct','not in data')}%\n"
                f"7D Change:            {info.get('price_change_7d_pct','not in data')}%\n"
                f"30D Change:           {info.get('price_change_30d_pct','not in data')}%\n"
                f"ATH:                  {info.get('ath','not in data')} ({info.get('ath_change_pct','not in data')}% from ATH)\n"
                f"Circulating Supply:   {fmt_number(info.get('circulating_supply'))}\n"
                f"Total Supply:         {fmt_number(info.get('total_supply'))}\n"
                f"24H Volume (USD):     {fmt_number(info.get('total_volume_usd'))}\n"
                f"{tech_block}"
                f"{mtf_summary}\n"
                f"══ END AUTHORITY DATA ══\n\n"
                f"NOTE: Cryptocurrency — no P/E, earnings, or moat. Use network/adoption metrics, "
                f"supply dynamics, on-chain flow (NVT, MVRV, S2F, realized cap) where possible. "
                f"If those metrics are not in AUTHORITY DATA above, write 'not in data'.\n\n"
            ...
            ic_context = (
                ...
                f"══ AUTHORITY DATA ══  (cite ONLY from this block — do not invent or recall values)\n"
                f"Price ({info.get('currency','USD')}): {price}\n"
                f"Market Cap:           {fmt_number(info.get('market_cap'))}\n"
                f"P/E Ratio:            {info.get('pe_ratio','not in data')}\n"
                f"P/B Ratio:            {info.get('pb_ratio','not in data')}\n"
                f"ROE:                  {pct(info.get('roe'))}\n"
                f"Revenue Growth:       {pct(info.get('revenue_growth'))}\n"
                f"Net Margin:           {pct(info.get('net_margin'))}\n"
                f"Gross Margin:         {pct(info.get('gross_margin'))}\n"
                f"Operating Margin:     {pct(info.get('operating_margin'))}\n"
                f"D/E Ratio:            {info.get('debt_equity','not in data')}\n"
                f"Current Ratio:        {info.get('current_ratio','not in data')}\n"
                f"Free Cash Flow:       {fmt_number(info.get('free_cashflow'))}\n"
                f"Beta:                 {info.get('beta','not in data')}\n"
                f"Analyst Target:       {info.get('analyst_target','not in data')}\n"
                f"Analyst Rec:          {info.get('recommendation','not in data')}\n"
                f"{tech_block}"
                f"{mtf_summary}\n"
                f"══ END AUTHORITY DATA ══\n\n"
                f"RESEARCH ANALYST SUMMARY:\n{research_output[:1500]}\n\n"
